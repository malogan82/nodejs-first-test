const express = require('express')
const bodyParser = require('body-parser')
const cors = require('cors')
const http = require('http')
const qs = require('querystring')
const calculator = require('./calculator')
const mongoDBconnection = require('./mongoDB.connection')

/*const server = http.createServer(function(request, response) {
  console.dir(request.param)

  if (request.method == 'POST') {
    console.log('POST')
    var body = ''
    request.on('data', function(data) {
      body += data
    })

    request.on('end', function() {
      const post = qs.parse(body)
      const numbers = post.numbers
      const result = calculator.add(numbers)
      response.writeHead(200, {'Content-Type': 'text/html'})
      response.end('Result: ' + result)
    })
  } else {
    var html = `
            <html/>
                <body/>
                    <form method="post" action="http://localhost:3000"/>Numbers: 
                        <input type="text" name="numbers" />
                        <input type="submit" value="Add" />
                    </form>
                </body>
            </html>`
    response.writeHead(200, {'Content-Type': 'text/html'})
    response.end(html)
  }
})

const port = 3000
const host = '127.0.0.1'
server.listen(port, host)
console.log(`Listening at http://${host}:${port}`)*/
const app = express()
const port = 3000
app.use(cors())
app.use(bodyParser.urlencoded({ extended: false }))
app.use(bodyParser.json())
app.get('/users', (req, res) => {
  console.log('Retrieving all users')
  mongoDBconnection.userModel.find({}, (err, users) => {
    users.forEach(user => {
      console.log('User name: '+user.name)
      console.log('User surname: '+user.surname)
      console.log('User age: '+user.age)
    })
    res.send(users)
  });
});
app.post('/save-user', (req, res) => {
  console.log('User name: '+req.body.name)
  console.log('User surname: '+req.body.surname)
  console.log('User age: '+req.body.age)
  var user = new mongoDBconnection.userModel({name: req.body.name, surname: req.body.surname, age: req.body.age})
  user.save(function(err, user){
    if (err) {
      return console.error(err);
    }  
    console.log("User inserted correctly!");
  });
  res.send(user)
})
app.listen(port, () => console.log(`User app listening on port ${port}!`))

